...
export const PatientMonitorPage: React.FC = () => {
  ...
  const animate = (time: number) => {
    ...
    const rate = 250 * playbackSpeedRef.current;
  ...
  const displayDeviceId = deviceId !== "MENUNGGU PERANGKAT..." ? deviceId : syncedDeviceId || (onlineDevices.length > 0 ? onlineDevices[0].name : "MENUNGGU PERANGKAT...");
  ...
  return (
    <div className="bg-clinical-surface/30 text-clinical-charcoal antialiased min-h-screen flex flex-col transition-colors duration-700 relative">
      ...
      <div className="relative z-10 flex flex-col flex-1">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-clinical-charcoal/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white border border-clinical-charcoal/5 rounded-[2rem] p-8 shadow-2xl max-w-md w-full mx-4 animate-in zoom-in-50 fade-in duration-500 ease-spring text-center">
              ...
              <div className="flex flex-col gap-3">
                ...
                <button onClick={() => setShowDeviceWarning(false)} className="w-full py-3.5 rounded-lg font-bold text-[13px] bg-clinical-surface hover:brightness-95 text-clinical-charcoal transition-colors">
                  Abaikan
                </button>
              </div>
            </div>
        ...
        <AlertPanel visible={showAlert} title={`KRITIS: ${alertTitle}`} onClose={() => setShowAlert(false)} />
        ...
        <main className="w-full max-w-container-max pt-8 pb-16 mx-auto px-margin-mobile md:px-margin-desktop flex flex-col gap-8 flex-1">
          <section className="w-full flex flex-col gap-5 flex-1">
            <div className="bg-white rounded-[2rem] px-6 py-4 flex flex-wrap justify-between items-center shadow-[0px_20px_40px_rgba(0,0,0,0.04)] border border-clinical-charcoal/5 gap-4 transition-all duration-700">
              <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto overflow-x-auto custom-scrollbar pb-2 lg:pb-0"></div>
              ...
              <div className="flex-shrink-0 bg-white border-t border-clinical-charcoal/5 px-6 py-3.5 flex flex-col xl:flex-row items-center justify-between gap-4 z-20 select-none">
                ...
                <div className="flex items-center gap-3 w-full xl:w-auto justify-between xl:justify-start">
                  <div className="flex items-center gap-2">
                    <button onClick={handlePlayPause} className="flex items-center justify-center bg-clinical-blue text-white rounded-full w-10 h-10 hover:bg-clinical-blue/90 active:scale-95 transition-all shadow-sm outline-none">
                      <span className="material-symbols-outlined text-[22px] font-bold">{isPlaying ? "pause" : "play_arrow"}</span>
                    </button>
                    ...
                    <div className="relative z-[9999]">
                      ...
                      >
                        <span className="material-symbols-outlined text-[16px]">{filterConfig.baselineBlocker || filterConfig.hfDenoise || filterConfig.bandpass || filterConfig.zScoreNorm ? "filter_alt" : "filter_alt_off"}</span>
                      </button>
                  ...
                  <div className="ml-2 flex flex-col justify-center">
                    <span className="text-[9px] font-bold text-clinical-charcoal/40 uppercase tracking-wider leading-none">WAKTU REKAMAN</span>
                ...
                <div className="flex-1 flex items-center gap-3 w-full px-2">
                  ...
                  />
                  <span className="text-xs font-bold text-clinical-charcoal/50 w-10 text-right font-mono-data">{Math.round((visibleCount / TOTAL_SAMPLES) * 100)}%</span>
                </div>
                ...
                <div className="flex items-center gap-3 flex-wrap justify-end w-full xl:w-auto">
                  ...
                  <div className="flex items-center gap-1.5 bg-clinical-surface p-1 rounded-full border border-clinical-charcoal/5">
                    <span className="text-[9px] font-bold text-clinical-charcoal/40 uppercase tracking-wider px-2 select-none">Gain</span>
                    <button
                      onClick={() => setGain(5)}
                  ...
                  {/* Paper Speed Config */}
                  <div className="flex items-center gap-1.5 bg-clinical-surface p-1 rounded-full border border-clinical-charcoal/5">
                    <span className="text-[9px] font-bold text-clinical-charcoal/40 uppercase tracking-wider px-2 select-none">Speed</span>
                    <button
                  ...
                  <div className="flex items-center gap-1 bg-clinical-surface p-1 rounded-full border border-clinical-charcoal/5">
                    <span className="text-[9px] font-bold text-clinical-charcoal/40 uppercase tracking-wider px-2 select-none">Playback</span>
                    <button
                      onClick={() => {
