...
const SessionPatientAvatar: React.FC<{ patientId: string | number; patientName: string }> = ({ patientId, patientName }) => {
  ...
  return photo ? <img src={photo} alt={patientName || ""} className="w-full h-full object-cover" /> : <>{patientName ? patientName.substring(0, 2) : "UK"}</>;
};
          ...
          <div className="relative max-w-4xl max-h-[90vh] w-full p-4 flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-end gap-3 mb-4">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-clinical-charcoal/50 hover:bg-clinical-charcoal/80 text-white rounded-full font-bold text-xs transition-colors backdrop-blur-md"
              >
          ...
          <div className="bg-white rounded-[2rem] border border-clinical-charcoal/5 shadow-[0px_20px_40px_rgba(0,0,0,0.04)] p-4 md:px-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 justify-end w-full md:w-auto">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {connectedPatients.map((p) => (
                      <option key={p.id || p.raw_id} value={p.raw_id || p.id}>
                        {p.name}
                      </option>
                    ))}
                  </select>
              ...
              {sessionId && (
                <button
                  onClick={() => navigate(-1)}
                  className="text-[11px] font-bold uppercase tracking-wider text-clinical-charcoal/70 hover:text-clinical-blue bg-clinical-surface border border-clinical-charcoal/5 hover:border-clinical-blue/20 px-5 py-2.5 rounded-full transition-all duration-700 flex items-center gap-2 shadow-sm hover:shadow-md"
                >
                  ...
                  <div className="space-y-3">
                    {allSessions.length === 0 ? (
                      <div className="bg-white border border-clinical-charcoal/5 p-8 rounded-[2rem] flex flex-col items-center justify-center shadow-[0px_20px_40px_rgba(0,0,0,0.04)] text-center">
                        ...
                        <p className="text-sm font-medium text-clinical-charcoal/60">{totalSessions === 0 ? "Belum ada riwayat sesi yang tersimpan." : "Tidak ada sesi untuk pasien yang dipilih."}</p>
                      </div>
                    ) : (
                          ...
                          >
                            <div className="flex items-center gap-5">
                              <div className="w-14 h-14 rounded-full bg-clinical-surface group-hover:bg-clinical-blue/10 transition-colors duration-700 flex items-center justify-center text-lg font-bold text-clinical-blue uppercase overflow-hidden flex-shrink-0 relative z-10">
                                {session.patient_id ? <SessionPatientAvatar patientId={session.patient_id} patientName={session.patient_name || ""} /> : session.patient_name ? session.patient_name.substring(0, 2) : "UK"}
                              </div>
                              <div className="relative z-10">
                                ...
                                <p className="text-xs font-medium text-clinical-charcoal/60 mt-0.5">
                                  Sesi: {session.id.substring(0, 8)}... • SN: {session.device_id}
                                </p>
                            ...
                            <div className="flex items-center gap-3">
                              <div className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider relative z-10 ${validationClass}`}>{validationStatus}</div>
                              {session.ecg_paper ? (
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setPreviewImage(API_URL + session.ecg_paper);
                                  }}
                ...
                <div className="bg-white border border-clinical-charcoal/5 rounded-[2rem] p-5 flex flex-wrap justify-between items-center shadow-[0px_20px_40px_rgba(0,0,0,0.04)] gap-4">
                  ...
                  <div className="ml-auto flex items-center">
                    {currentSegment?.confirmation !== null && currentSegment?.confirmation !== undefined ? (
                      <div className="px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-signal-green/10 text-signal-green border border-signal-green/20">Sudah Divalidasi</div>
                    ) : (
                      <div className="px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-clinical-surface text-clinical-charcoal/60 border border-clinical-charcoal/10">Belum Divalidasi</div>
                    )}
                  </div>
                </div>
