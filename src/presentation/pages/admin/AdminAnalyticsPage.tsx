...
export const AdminAnalyticsPage: React.FC = () => {
  ...
  const sessionsUrl =
    selectedPatientFilter === "ALL"
      ? role === "dokter"
        ? `/api/sessions?doctor_id=${userId}&page=${currentPage}&limit=${itemsPerPage}`
  ...
  useEffect(() => {
    if (allSessions.length > 0) {
      ...
      if (sessionIds.length > 0) {
        supabase.rpc("get_sessions_validation_counts", { session_ids: sessionIds }).then(({ data: stats, error }) => {
          if (!error && stats) {
  ...
  useEffect(() => {
    ...
    validData.forEach((payload: any, i: number) => {
      ...
      const frameResult = calculateFrameHeartRate(extractRawFromPayload(payload), FILTERS_CLINICAL_DEFAULT);
      const calculatedHR = frameResult.bpm > 0 ? frameResult.bpm : payload.validation?.hr || payload.heart_rate || (i > 0 ? loadedSegments[i - 1].heartRate : "--");
        ...
        <header className="sticky top-0 bg-clinical-surface/80 backdrop-blur-xl border-b border-clinical-charcoal/5 z-40 px-4 md:px-6 py-4 flex justify-between items-center w-full w-full transition-all duration-300">
          <div className="flex items-center gap-3">
            <button
              ...
              className="md:hidden flex items-center justify-center p-2 -ml-2 rounded-full hover:bg-clinical-surface text-clinical-charcoal/70 transition-colors outline-none"
              title="Sembunyikan / Tampilkan Menu Utama"
            >
        ...
        <div className="w-full px-4 md:px-6 pt-4 w-full z-30 relative animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
          <div className="bg-white/80 backdrop-blur-md rounded-[1.5rem] border border-clinical-charcoal/5 shadow-sm p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all duration-700 hover:shadow-md">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-clinical-charcoal/70">
                  {!sessionId
                    ? "Pilih sesi dari daftar di bawah untuk melihat detail rekaman."
                    : currentSessionMeta
                      ? `Pasien: ${currentSessionMeta.patient_name || "Anonim"} | Mulai: ${new Date(currentSessionMeta.started_at).toLocaleString("id-ID")} | Sesi: ${sessionId}`
                      : `Menampilkan detail rekaman EKG untuk Sesi: ${sessionId}`}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              {!sessionId && (
                ...
                >
                  ...
                  {connectedPatients.map((p) => (
                    <option key={p.id || p.raw_id} value={p.id || p.raw_id}>
                      {p.name}
                    </option>
                  ))}
                </select>
              ...
              <button
                onClick={() => navigate(-1)}
                className="text-xs font-body-sm font-headline-md text-clinical-charcoal/70 hover:text-clinical-blue border border-clinical-blue/20 px-4 py-2 rounded-lg hover:border-clinical-blue transition-all"
              >
                Kembali
              </button>
            </div>
              ...
              <div className="mt-4 mx-auto w-full max-w-container-max px-4 md:px-6 flex-1 animate-in fade-in slide-in-from-bottom-6 duration-700 delay-100 ease-out fill-mode-both">
                <div className="space-y-4">
                  {allSessions.length === 0 ? (
                    <div className="bg-white border border-clinical-blue/20/60 p-5 rounded-xl flex items-center justify-center shadow-sm">
                      <p className="text-sm font-body-sm text-clinical-charcoal/70">{totalSessions === 0 ? "Belum ada riwayat sesi yang tersimpan." : "Tidak ada sesi untuk pasien yang dipilih."}</p>
                    </div>
                  ) : (
                    allSessions.map((session: any) => {
                      ...
                      return (
                        <div
                          key={session.id}
                          className="bg-white border border-clinical-charcoal/5 p-4 md:p-5 rounded-[1.5rem] flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:shadow-lg hover:border-clinical-blue/20 transition-all duration-500 cursor-pointer group"
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-xs font-body-sm text-clinical-charcoal/70 font-mono-data mt-0.5">
                                Sesi: {session.id.substring(0, 8)}... â€¢ SN: {session.device_id}
                              </p>
                              <p className="text-[10px] text-clinical-charcoal/70 mt-1 font-headline-md">{new Date(session.started_at).toLocaleString("id-ID")}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                            <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${validationClass}`}>{validationStatus}</div>
                ...
                {totalSessions > itemsPerPage && (
                  <div className="mt-4 mb-12">
                    <Pagination currentPage={currentPage} totalItems={totalSessions} itemsPerPage={itemsPerPage} onPageChange={setCurrentPage} />
                  </div>
                )}
              </div>
            );
                ...
                {isEditingSessionNote ? (
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex justify-end gap-2">
                      <button onClick={() => setIsEditingSessionNote(false)} className="px-4 py-2 text-xs font-bold text-on-surface-variant hover:bg-surface-variant/30 rounded-lg transition-colors" disabled={isSubmittingSessionNote}>
                        Batal
                      </button>
                      {sessionDevNote && (
                        <button onClick={deleteSessionNote} className="px-4 py-2 text-xs font-bold text-white bg-alert-red hover:bg-alert-red/90 rounded-lg transition-colors flex items-center gap-1" disabled={isSubmittingSessionNote}>
                      ...
                      >
                        {isSubmittingSessionNote ? <span className="material-symbols-outlined text-[14px] animate-spin">sync</span> : <span className="material-symbols-outlined text-[14px]">save</span>}
                  ...
                  <div className="flex items-start justify-between gap-4 p-3 bg-slate-50 border border-outline-variant/30 rounded-lg">
                    ...
                    <button
                      onClick={() => {
                        setEditSessionNoteValue(sessionDevNote);
              ...
              <div className="bg-white border border-clinical-blue/20 rounded-xl p-3 flex flex-wrap justify-between items-center shadow-sm gap-3">
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-sm font-body-sm font-headline-md text-clinical-charcoal flex items-center gap-2">
                    Waktu Rekaman:
                    <span className="px-2 py-1 bg-white-container-high rounded text-clinical-blue font-mono-data text-xs font-body-sm">{currentEvent ? `${currentEvent.timeStr} - ${events[selectedIdx + 1]?.timeStr || "Akhir"}` : "--"}</span>
                  </span>
                </div>
                <div className="ml-auto flex items-center">
                  {currentSegment?.confirmation !== null && currentSegment?.confirmation !== undefined ? (
                    <div className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-signal-green/10 text-signal-green border border-signal-green/20">Sudah Divalidasi</div>
                  ) : (
                    <div className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-clinical-surface text-clinical-charcoal/60 border border-outline-variant">Belum Divalidasi</div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white border border-outline-variant/60 rounded-xl p-4 shadow-sm flex flex-col gap-2">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-on-surface-variant font-bold uppercase tracking-wider flex items-center gap-1">
                    <span className="material-symbols-outlined text-[14px]">device_thermostat</span> Created At (IoT)
                  </span>
              ...
              <div className="bg-charcoal text-white rounded-xl p-5 shadow-xl flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-white/10 p-3 rounded-lg border border-white/5">
                    ...
                    <p className={`text-sm font-bold ${aiMetrics?.confidence_percent != null ? (aiMetrics.confidence_percent >= 95 ? "text-green-400" : "text-white/80") : "text-white/50"}`}>
                      {aiMetrics?.confidence_percent != null ? `${aiMetrics.confidence_percent.toFixed(2)}%` : "--"}
                    </p>
                  </div>
                ...
                <div className="text-[9px] text-white/40 text-center uppercase tracking-widest mt-1">Runtime: {aiMetrics?.runtime || "UNKNOWN"}</div>
              </div>
              ...
              <div className="bg-clinical-surface border border-clinical-blue/20 rounded-xl p-5 shadow-sm flex flex-col gap-4">
                ...
                <div className="flex justify-between items-center bg-white p-3 rounded-lg border border-outline-variant/30 shadow-sm">
                  <div>
                    ...
                    <p className="text-sm font-bold mt-0.5">
                      {currentSegment?.confirmation === true ? (
                        <span className="text-signal-green">Benar</span>
                      ) : currentSegment?.confirmation === false ? (
                        <span className="text-alert-red">Salah</span>
                      ) : (
                ...
                {currentSegment?.confirmation !== undefined && currentSegment?.confirmation !== null && (
                  <div className="bg-white p-3 rounded-lg border border-outline-variant/30 shadow-sm">
                    ...
                    <p className="text-lg font-extrabold text-clinical-blue mt-0.5">{currentSegment?.docClassification || "--"}</p>
                  </div>
                )}

                <div className="bg-white p-3 rounded-lg border border-outline-variant/30 shadow-sm">
                  <p className="text-[10px] text-on-surface-variant font-bold uppercase tracking-wider mb-1.5">Catatan Dokter</p>
                  <p className={`text-sm ${currentSegment?.docNote ? "text-charcoal italic" : "text-on-surface-variant/50"}`}>{currentSegment?.docNote ? `"${currentSegment.docNote}"` : "Tidak ada catatan."}</p>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-y-4 gap-x-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-mono font-semibold text-charcoal">
                        {system.memory_usage_mb ? `${system.memory_usage_mb} MB` : "--"} <span className="text-xs text-on-surface-variant font-normal">({system.memory_usage_percent?.toFixed(1) || "--"}%)</span>
                      </p>
                    </div>
                    <div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-on-surface-variant mt-1">
                        Status saat ini: {currentSegment.hidden ? <span className="text-alert-red font-bold">Disembunyikan (Hidden)</span> : <span className="text-signal-green font-bold">Terlihat (Visible)</span>}
                      </p>
                    </div>
                    <button
                      onClick={async () => {
