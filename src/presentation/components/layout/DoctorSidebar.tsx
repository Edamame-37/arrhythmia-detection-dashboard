    ...
    return (
        <>
            ...
            <aside id="main-sidebar" className={`
                ...
                md:fixed md:left-0 md:top-0 md:h-screen md:w-[260px] md:bg-surface-container-lowest md:border-r md:border-outline-variant md:translate-x-0 md:translate-y-0 md:flex md:rounded-none md:shadow-none md:opacity-100
                ...
                ${isOpen ? 'opacity-100 scale-100 flex translate-y-0' : 'opacity-0 scale-95 pointer-events-none hidden md:flex'}
            `}>
                <div className="hidden md:flex p-6 items-center gap-3 border-b border-outline-variant/30 cursor-pointer" onClick={() => { navigate('/doctor/dashboard'); closeSidebar(); }}>
                ...
                <nav className="flex-1 px-4 mt-6 space-y-1">
                    <Link onClick={closeSidebar} className={`flex items-center gap-3 px-4 py-3 rounded-lg font-semibold shadow-sm transition-all ${isActive('/doctor/dashboard') ? 'bg-medical-teal text-white' : 'text-on-surface-variant hover:bg-surface-container-low group'}`} to="/doctor/dashboard">
                    ...
                    <Link onClick={closeSidebar} className={`flex items-center gap-3 px-4 py-3 rounded-lg font-semibold shadow-sm transition-all ${isActive('/doctor/qr-scanner') ? 'bg-medical-teal text-white' : 'text-on-surface-variant hover:bg-surface-container-low group'}`} to="/doctor/qr-scanner">
                    ...
                    <Link onClick={closeSidebar} className={`flex items-center gap-3 px-4 py-3 rounded-lg font-semibold shadow-sm transition-all ${isActive('/doctor/monitor') ? 'bg-medical-teal text-white' : 'text-on-surface-variant hover:bg-surface-container-low group'}`} to="/doctor/monitor">
                    ...
                    <Link onClick={closeSidebar} className={`flex items-center gap-3 px-4 py-3 rounded-lg font-semibold shadow-sm transition-all ${isActive('/doctor/analytics') ? 'bg-medical-teal text-white' : 'text-on-surface-variant hover:bg-surface-container-low group'}`} to="/doctor/analytics">
                ...
                <div className="p-4 mt-4 md:mt-0 border-t border-outline-variant/40 md:bg-surface-container-low/50">
                    <div className="flex bg-surface border border-outline-variant/50 p-3 rounded-lg items-center gap-3 transition-all group hover:border-medical-teal cursor-pointer" onClick={() => { navigate('/doctor/profile'); closeSidebar(); }}>
